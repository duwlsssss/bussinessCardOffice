...
const Player=({controlsRef})=>{
    ...
    useFrame((state,delta)=>{
        if(!isFocused){
            ...
            state.camera.position.lerp(targetCameraPosition, delta * 25);
        ...
        <>
            <RigidBody 
                ...
                <CapsuleCollider 
                    args={[20,27]} 
                    position={[0,52,1600]}
                    // position={[-10,52,450]}//내부 테스트용
                    onCollisionEnter={(other)=>{
                        if(other.rigidBodyObject.name==="out_floor"||"in_floor"){
